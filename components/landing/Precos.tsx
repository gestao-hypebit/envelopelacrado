    ...
    <section className="py-32 px-6 lg:px-8" style={{ background: '#FAFAF8' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          className="grid lg:grid-cols-2 gap-0 rounded-3xl overflow-hidden shadow-xl border"
          style={{ borderColor: '#F5C6D4' }}>

          {/* LEFT — preço */}
          <div className="relative p-10 flex flex-col justify-between overflow-hidden"
            style={{ background: 'linear-gradient(145deg, #FEF2F5, #FFF8F2)' }}>

            <div className="absolute -top-16 -right-16 w-56 h-56 rounded-full opacity-30 blur-3xl"
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-end gap-1 mb-2">
                ...
                <span className="font-display font-bold leading-none" style={{ fontSize: '6rem', color: '#1a0e14' }}>19</span>
                <span className="text-3xl font-bold mb-3" style={{ color: '#1a0e14' }}>,90</span>
              </div>
              <p className="text-sm mb-8" style={{ color: '#A0785A' }}>
                ...
                {garantias.map((g) => (
                  <div key={g.texto} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                      style={{ background: 'white', border: '1px solid #F5C6D4' }}>
                      <g.icon className="w-4 h-4" style={{ color: '#C9768F' }} />
                    </div>
                    <span className="text-sm" style={{ color: '#7a6070' }}>{g.texto}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative z-10">
              ...
              <p className="text-center text-xs mt-3" style={{ color: '#A0785A' }}>
              ...
              {inclusos.map((item, i) => (
                <motion.li key={item}
                  ...
                  <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                    style={{ background: '#FEF2F5', border: '1px solid #F5C6D4' }}>
                    <Check className="w-3 h-3" style={{ color: '#C9768F' }} />
                  </div>
                  <span className="text-sm" style={{ color: '#7a6070' }}>{item}</span>
                </motion.li>
              ))}
            </ul>

            <div className="mt-8 pt-8 border-t" style={{ borderColor: '#F5EDE3' }}>
