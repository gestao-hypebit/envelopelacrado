  ...
  return (
    <div className="relative">
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b flex-shrink-0" style={{ background: '#0a0406', borderColor: '#1a0808' }}>
          ...
          <div className="flex-1 flex justify-center">
            <div className="px-3 py-1 rounded text-xs" style={{ background: '#1a0808', color: '#6a5858', fontFamily: 'monospace' }}>
              🔒 envelopelacrado.com.br/p/<span style={{ color: '#C9768F', opacity: 0.9 }}>ana-e-joao-k3m9</span>
            </div>
          </div>
      ...
      <motion.div animate={{ y: [-4, 4, -4] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
        ...
        style={{ background: 'rgba(30,8,24,0.95)', borderColor: 'rgba(201,118,143,0.3)', zIndex: 10 }}>
        <p className="text-xs font-semibold" style={{ color: '#C9768F' }}>💌 Surpresa pronta!</p>
        <p className="text-xs" style={{ color: 'rgba(240,228,212,0.55)' }}>Entregue por QR Code</p>
      </motion.div>
      <motion.div animate={{ y: [4, -4, 4] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
        ...
        style={{ background: 'rgba(30,8,24,0.95)', borderColor: 'rgba(201,169,110,0.3)', zIndex: 10 }}>
        <p className="text-xs font-semibold" style={{ color: '#C9A96E' }}>✨ IA gerou em segundos</p>
        <p className="text-xs" style={{ color: 'rgba(240,228,212,0.55)' }}>Narrativa única</p>
      </motion.div>
    </div>
  )
}
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden"
      ...
      {[
        ...
        <motion.div key={i} className="absolute pointer-events-none"
          style={{ width: orb.w, height: orb.h, left: orb.x, top: orb.y, background: orb.color, borderRadius: '50%', filter: 'blur(90px)', opacity: 0.12, transform: 'translate(-50%,-50%)' }}
          animate={{ x: [0,20,-12,8,0], y: [0,-16,14,-6,0], opacity: [0.12,0.18,0.08,0.14,0.12] }}
      ...
      {[...Array(18)].map((_, i) => (
        <div key={i} className="absolute rounded-full pointer-events-none"
          style={{
            ...
            left: `${(i * 37 + 5) % 95}%`,
            top: `${(i * 53 + 8) % 90}%`,
            background: i % 2 === 0 ? '#C9A96E' : '#C9768F',
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-32 grid lg:grid-cols-2 gap-8 lg:gap-16 items-center w-full">
        ...
        <div>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            ...
            <span className="text-xs font-medium" style={{ color: 'rgba(240,228,212,0.7)' }}>+2.000 casais emocionados</span>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            ...
            <div className="flex -space-x-3">
              {['#C9768F', '#C9A96E', '#9B8EC4', '#7D9B76'].map((c, i) => (
                <div key={i} className="w-9 h-9 rounded-full border-2 flex items-center justify-center text-xs"
                  style={{ background: c, borderColor: '#0d0612' }}>
                  {['😍', '🥰', '❤️', '💕'][i]}
                </div>
              ))}
            </div>
            <div>
              <p className="text-sm font-semibold" style={{ color: '#F0E4D4' }}>Mais de 2.000 histórias criadas</p>
              <p className="text-xs" style={{ color: 'rgba(240,228,212,0.5)' }}>Dia dos Namorados, aniversários, pedidos de namoro</p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity }}
        ...
        <p className="text-xs tracking-widest uppercase" style={{ color: 'rgba(201,118,143,0.3)' }}>scroll</p>
        <div className="w-px h-8" style={{ background: 'linear-gradient(to bottom, rgba(201,118,143,0.4), transparent)' }} />
      </motion.div>
    </section>
  )
}
